...
#include <vector>
#include <iostream>
class FastintTableProducer : public edm::stream::EDProducer<> {
public:
  FastintTableProducer(edm::ParameterSet const& params)
      : name_(params.getParameter<std::string>("name")), extension_(params.getParameter<bool>("extension")) {
        std::cout<<"Added FastintTableProducer"<<std::endl;
    ...
    for (const std::string& vname : varsPSet.getParameterNamesForType<edm::ParameterSet>()) {
      ...
      if (type == "int")
        ...
        vars_.push_back(std::make_unique<FloatVectorVar>(vname, varPSet, consumesCollector()));
      else if (type == "double")
    ...
    produces<nanoaod::FlatTable>("sslTable");
  }

  ~FastintTableProducer() override {}
  ...
  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    variable.ifValue(edm::ParameterDescription<std::string>(
                     ...
                     edm::allowedValues<std::string>(
                         "int", "float", "double", "std::vector<float>","bool", "candidatescalarsum", "candidatesize", "candidatesummass"));
    ...
    descriptions.add("FastintTableProducer",desc);
  }
  void produce(edm::Event& iEvent, const edm::EventSetup& iSetup) override {
    auto out = std::make_unique<nanoaod::FlatTable>(40, this->name_, false);

    for (const auto& var : vars_)
      var->fill(iEvent, *out);

    iEvent.put(std::move(out),"sslTable");
  }
...
protected:
  ...
  public:
    VariableT(const std::string& aname, const edm::ParameterSet& cfg, edm::ConsumesCollector&& cc)
        : Variable(aname, cfg), src_(cc.consumes<std::vector<int>>(cfg.getParameter<edm::InputTag>("src"))) {}
    ~VariableT() override {}
    void fill(const edm::Event& iEvent, nanoaod::FlatTable& out) const override {
          edm::Handle<std::vector<int>> vecHandle;
          iEvent.getByToken(src_, vecHandle);
          const std::vector<int>& vec = *vecHandle;
          out.template addColumn<int>(
          this->name_, vec, this->doc_);
    }

  protected:
    edm::EDGetTokenT<std::vector<int>> src_;
  };
  typedef VariableT<int> IntVar;
  typedef VariableT<float> FloatVar;
  typedef VariableT<std::vector<float>> FloatVectorVar;
...
DEFINE_FWK_MODULE(FastintTableProducer);
