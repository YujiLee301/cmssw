...
#include <vector>
#include <iostream>
class FastVariablesTableProducer : public edm::stream::EDProducer<> {
public:
  FastVariablesTableProducer(edm::ParameterSet const& params)
      : name_(params.getParameter<std::string>("name")), extension_(params.getParameter<bool>("extension")) {
        std::cout<<"Added FastVariablesTableProducer"<<std::endl;
  ...
  ~FastVariablesTableProducer() override {}
...
protected:
  ...
  public:
    VariableT(const std::string& aname, const edm::ParameterSet& cfg, edm::ConsumesCollector&& cc)
        : Variable(aname, cfg), src_(cc.consumes<std::vector<float>>(cfg.getParameter<edm::InputTag>("src"))) {}
    ~VariableT() override {}
...
DEFINE_FWK_MODULE(FastVariablesTableProducer);
